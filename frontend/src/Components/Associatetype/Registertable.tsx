...
export default function Dashboardholiday() {
  ...
  const typeofschema = {
    associateType: { type: "String", label: "Associate Type" },
    refferedBy: { type: "Checkbox", label: "Reffered By" },
    requisitionTo: { type: "Checkbox", label: "Requisition To" },
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/associatetype/allassociatetype/${User?._id}`)
      .then((response) => {
    ...
    setConfig({
      ...
      searchPlaceholder: "Search Associate Type...",
      userAvatar: userAvatar, // Use the imported avatar
      tableColumns: {
        title: "Associate Type",
        description: "Manage Associate Type and view their details.",
        headers: [
          { label: "Associate Type", key: "description" },
          { label: "Reffered By", key: "reffered" },
          { label: "Requisition To", key: "requisition" },
  ...
  const mappedTableData = data?.map((item) => {
    return {
      _id: item?._id,
      description: item?.associateType || "Associate Type not provided",
      reffered: item?.refferedBy
        ? "True"
        : "False" || "Refference  not provided",
      requisition: item?.requisitionTo
        ? "True"
        : "False" || "Requisition not provided",
      delete: `/associatetype/delete/${item?._id}`,
