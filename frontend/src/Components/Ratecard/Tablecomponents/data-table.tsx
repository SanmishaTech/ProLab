...
const generateMockHistory = <T extends {
  ...
  const currentPurchaseRate = item.purchaseRate || 0;
  const currentSaleRate = item.saleRate || 0;
  ...
  return [
    ...
    },
    {
      ...
      purchaseRate: Math.round(currentPurchaseRate * 1.05),
      saleRate: Math.round(currentSaleRate * 1.05),
      associate: item.associate,
      department: item.department,
    ...
    },
    {
      ...
      purchaseRate: originalPurchaseRate,
...
const formatCellValue = <T extends Record<string, any>>(
  ...
  if (column === "purchaseRate" || column === "saleRate") {
...
>({
  ...
  const handleBulkEditConfirm = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    ...
    if (onBulkEdit) {
      const updatedItems = selectedItems.map(item => {
        ...
        const originalPurchaseRate = typeof item?.originalPurchaseRate === 'number' && !isNaN(item.originalPurchaseRate) 
          ? item.originalPurchaseRate
          : (typeof item?.purchaseRate === 'number' && !isNaN(item.purchaseRate) ? item.purchaseRate : 0);
        
        const originalSaleRate = typeof item?.originalSaleRate === 'number' && !isNaN(item.originalSaleRate)
          ? item.originalSaleRate
          : (typeof item?.saleRate === 'number' && !isNaN(item.saleRate) ? item.saleRate : 0);
        
        // Apply percentage discount if specified
        if (discountPercentage > 0) {
          updatedItem.purchaseRate = originalPurchaseRate * (1 - discountPercentage / 100);
          updatedItem.saleRate = originalSaleRate * (1 - discountPercentage / 100);
        }
  ...
  }, [bulkEditPercentage, purchaseRateValue, saleRateValue, selectedItems, onBulkEdit]);
  ...
  const renderCell = useCallback((item: T, column: Column<T>) => {
    ...
    if (column.key === "purchaseRate" || column.key === "saleRate") {
      ...
      const numericValue = typeof value === 'number' && !isNaN(value) ? value as number : 0;
      
      return (
        <div className="flex items-center">
          {typeof value === 'number' ? 
            `${column.key === "purchaseRate" ? "₹" : "₹"}${numericValue.toFixed(2)}` :
            String(value || "0.00")
          }
        </div>
      );
  ...
  const renderHistoryContent = () => {
    ...
    testGroups.forEach(group => {
      group.history.sort((a, b) => {
        ...
        const dateA = a.fromDate ? new Date(a.fromDate).getTime() : 
          (a.date ? new Date(a.date.split('/').reverse().join('-')).getTime() : 0);
        const dateB = b.fromDate ? new Date(b.fromDate).getTime() : 
          (b.date ? new Date(b.date.split('/').reverse().join('-')).getTime() : 0);
        return dateB - dateA;
      });
    });
      ...
      <div className="w-full">
        <Tabs defaultValue="all" value={activeHistoryTab} onValueChange={setActiveHistoryTab}>
          <TabsList className="mb-4 flex flex-wrap">
            <TabsTrigger value="all">All Tests ({testGroups.length})</TabsTrigger>
            {associates.map(associate => (
              <TabsTrigger key={associate} value={associate || "unknown"}>
                {associate} ({testGroups.filter(g => g.test.associate === associate).length})
              </TabsTrigger>
            ))}
            {departments.map(department => (
              <TabsTrigger key={`dept-${department}`} value={`dept-${department}`}>
                {department} Dept ({testGroups.filter(g => g.test.department === department).length})
              </TabsTrigger>
            ))}
          </TabsList>
            ...
            <ScrollArea className="h-[500px] pr-4">
              {testGroups.length === 0 ? (
                ...
                testGroups.map((group, groupIndex) => (
                  <div key={`group-${groupIndex}`} className="mb-6 bg-muted/20 p-4 rounded-md">
                    <div className="flex justify-between items-center mb-3">
                      <h3 className="text-lg font-semibold">{group.test.name || `Test #${group.test._id}`}</h3>
                      <div className="flex items-center gap-2">
                        {group.test.department && (
                          <Badge variant="secondary">{group.test.department}</Badge>
                        )}
                        {group.test.associate && (
                          <Badge variant="outline">{group.test.associate}</Badge>
                        )}
                      </div>
                    </div>
                    
                    <div className="mb-2 text-sm flex flex-wrap gap-4 text-muted-foreground">
                      <span>Original Purchase Rate: ₹{(group.test.originalPurchaseRate || 0).toFixed(2)}</span>
                      <span>Original Sale Rate: ₹{(group.test.originalSaleRate || 0).toFixed(2)}</span>
                      <span>Current Purchase Rate: ₹{(group.test.purchaseRate || 0).toFixed(2)}</span>
                      <span>Current Sale Rate: ₹{(group.test.saleRate || 0).toFixed(2)}</span>
                    </div>
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableCell className="font-medium text-right">Purchase Rate</TableCell>
                            <TableCell className="font-medium text-right">Sale Rate</TableCell>
                        ...
                        <TableBody>
                          {group.history.map((entry, entryIndex) => {
                            ...
                            const purchaseRateDiff = prevEntry 
                              ? (entry.purchaseRate || entry.purchasePrice || 0) - (prevEntry.purchaseRate || prevEntry.purchasePrice || 0) 
                              : 0;
                            const saleRateDiff = prevEntry 
                              ? (entry.saleRate || 0) - (prevEntry.saleRate || 0) 
                              : 0;
                            
                            // Determine if this was an increase or decrease
                            const purchaseDirection = purchaseRateDiff > 0 
                              ? "text-green-500" 
                              : purchaseRateDiff < 0 
                                ? "text-red-500" 
                                : "text-muted-foreground";
                            
                            const saleDirection = saleRateDiff > 0 
                              ? "text-green-500" 
                              : saleRateDiff < 0 
                                ? "text-red-500" 
                                : "text-muted-foreground";
                            ...
                            const displayDate = entry.date || 
                              (entry.fromDate ? new Date(entry.fromDate).toLocaleDateString("en-GB", {
                                day: "2-digit",
                                month: "2-digit",
                                year: "numeric"
                              }) : "Unknown");
                            
                            const displayPurchaseRate = entry.purchaseRate || entry.purchasePrice || 0;
                            
                            return (
                              <TableRow key={`entry-${entryIndex}`}>
                                <TableCell className="whitespace-nowrap">{displayDate}</TableCell>
                                <TableCell className="text-right">₹{displayPurchaseRate.toFixed(2)}</TableCell>
                                <TableCell className="text-right">₹{(entry.saleRate || 0).toFixed(2)}</TableCell>
                                <TableCell>
                                  {prevEntry && (
                                    <div className="flex flex-col text-xs">
                                      <span className={purchaseDirection}>
                                        Purchase: {purchaseRateDiff > 0 ? "+" : ""}{purchaseRateDiff.toFixed(2)}
                                        {purchaseRateDiff !== 0 && (
                                          <span className="ml-1">
                                            ({Math.abs((purchaseRateDiff / (prevEntry.purchaseRate || prevEntry.purchasePrice || 1)) * 100).toFixed(1)}%)
                                          </span>
                                        )}
                                      </span>
                                      <span className={saleDirection}>
                                        Sale: {saleRateDiff > 0 ? "+" : ""}{saleRateDiff.toFixed(2)}
                                        {saleRateDiff !== 0 && (
                                          <span className="ml-1">
                                            ({Math.abs((saleRateDiff / (prevEntry.saleRate || 1)) * 100).toFixed(1)}%)
                                          </span>
                                        )}
                                      </span>
                                    </div>
          ...
          {/* Associate Tabs */}
          {associates.map(associate => (
            <TabsContent key={associate} value={associate || "unknown"} className="space-y-6">
              <ScrollArea className="h-[500px] pr-4">
                {testGroups
          ...
          {/* Department Tabs */}
          {departments.map(department => (
            <TabsContent key={`dept-${department}`} value={`dept-${department}`} className="space-y-6">
              <ScrollArea className="h-[500px] pr-4">
                {testGroups
      ...
      <div className="space-y-4">
        ...
        >
          <DialogContent className="sm:max-w-[425px] z-[100]">
            ...
            <DialogFooter>
              <Button onClick={() => handleSave(editingItem as T)}>Save changes</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
