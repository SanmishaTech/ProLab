...
export default function Dashboardholiday() {
  ...
  useEffect(() => {
    ...
    const fetchparameter = async () => {
      try {
        const response = await axios.get(
          `/api/collectionmaster/allcollectionmaster/${User?._id}`
        );
    ...
    const fetchparametergroup = async () => {
      try {
        const response = await axios.get(
          `/api/collectionmaster/allcollectionmaster/${User?._id}/${User?._id}`
        );
        console.log(response.data);
  ...
  useEffect(() => {
    // Fetch data from the API
    axios
      .get(`/api/collectionmaster/allcollectionmaster/${User?._id}`)
      .then((response) => {
        setData(response.data);
        console.log("This is BABA", response.data);
        setLoading(false);
      })
      .catch((err) => {
    ...
    setConfig({
      ...
      searchPlaceholder: "Search Collection Master...",
      userAvatar: userAvatar, // Use the imported avatar
      tableColumns: {
        title: "Collection Master",
        description: "Manage Collection Master and view their details.",
        headers: [
          { label: "Collection Name", key: "name" },
          { label: "Country", key: "price" },
          { label: "Email Id", key: "code" },
  ...
  const mappedTableData = data
    ? data?.map((item) => {
        ...
        return {
          _id: item?._id,
          name: item?.collectionName || "Test Name not provided",
