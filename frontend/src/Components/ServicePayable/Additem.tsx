...
const AddItem: React.FC<AddItemProps> = ({ onAdd, typeofschema }) => {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setError("Failed to add Service payable. Please try again.");
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Dialog open={handleopen} onOpenChange={setHandleopen}>
      <DialogTrigger asChild>
        <Button variant="outline">Add Service Payable </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Add New Service Payable </DialogTitle>
          <DialogDescription>
            Enter the details of the Service Payable you want to add.
          </DialogDescription>
        </DialogHeader>
