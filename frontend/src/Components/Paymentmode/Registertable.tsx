...
export default function Dashboardholiday() {
  ...
  const {
    ...
    endpoint: `/api/paymentMode/allpaymentmode/${User?._id}`,
    params: {
  ...
  useEffect(() => {
    ...
    setConfig({
      ...
      searchPlaceholder: "Search Payment Mode Master...",
      userAvatar: userAvatar, // Use the imported avatar
      tableColumns: {
        title: "Payment Mode Master",
        description: "Manage Payment Mode Master and view their details.",
        headers: [
          { label: "Payment Mode", key: "name" },
  ...
  const mappedTableData = data1
    ? data1?.map((item) => {
        ...
        return {
          _id: item?._id,
          name: item?.paymentMode || "Name not provided",
