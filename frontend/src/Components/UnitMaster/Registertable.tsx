...
export default function Dashboardholiday() {
  ...
  const {
    ...
    endpoint: `/api/unitmaster/allunitmaster/${User?._id}`,
    params: {
      queryKey: ["unitmaster"],
      queryKeyId: User?._id,
      retry: 5,
  ...
  useEffect(() => {
    ...
    setConfig({
      ...
      searchPlaceholder: "Search Unit Master...",
      userAvatar: userAvatar, // Use the imported avatar
      tableColumns: {
        title: "Unit Master",
        description: "Manage Unit Master and view their details.",
        headers: [
          { label: "Unit", key: "name" },
  ...
  const mappedTableData = data1
    ? data1?.map((item) => {
        ...
        return {
          _id: item?._id,
          name: item?.unit || "Name not provided",
