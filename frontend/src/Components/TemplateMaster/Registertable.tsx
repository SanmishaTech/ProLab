...
export default function Dashboardholiday() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/template/alltemplatemaster/${User?._id}`)
      .then((response) => {
    ...
    setConfig({
      ...
      searchPlaceholder: "Search Template Master...",
      userAvatar: "/path-to-avatar.jpg",
      tableColumns: {
        title: "Template Master",
        description: "Manage Template Master and view their details.",
        headers: [
          { label: "Template", key: "one" },
  ...
  const mappedTableData =
    Array.isArray(data) && data.length > 0
      ? data.map((item) => {
          ...
          return {
            _id: item?._id,
            one: item?.template || "Unknown",
            edit: `/template/update/${item?._id}`,
            delete: `/template/delete/${item?._id}`,
